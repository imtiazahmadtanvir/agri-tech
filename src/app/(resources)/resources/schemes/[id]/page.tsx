...
export default function Schemadetails({ params }) {
  const { id } = params
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const matchedItem = json.find(item => item.id === parseInt(id))
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
        ...
        <div className="w-full ">
         ...
         <p className="text-gray-600 italic text-xl">{scheme.name_en}</p>
          <p className="text-gray-600 mb-3 mt-3"><strong className='text-black'>Type:</strong> {scheme.type}</p>
          ...
          <p className="mb-1"><strong>Method:</strong> {scheme.application.method}</p>
          <p className="mb-1">
            <strong>Required Documents:</strong> {scheme.application.documents_required.join(', ')}
          </p>
          <p className="mb-4"><strong>Deadline:</strong> {scheme.application.deadline}</p>
